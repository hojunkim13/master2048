...
class Agent:
    ...
    def storeTransition(self, *transition):
        ...
        s = preprocessing(s).unsqueeze(0).float()
        s_ = preprocessing(s_).unsqueeze(0).float()
        a = torch.tensor(a).cuda()
        with torch.no_grad():
            pred_value = self.net(s.cuda())[0][a]
            target_value = r + self.gamma * torch.max(self.net_(s_.cuda()), dim=1)[0] * (not d)
    ...
    def learn(self):
        ...
        A = torch.tensor(list(data[1]), dtype=torch.int64).cuda().view(-1, 1)
        ...
        value = torch.gather(self.net(S), dim=1, index=A).squeeze()
    ...
    def softUpdate(self):
        for target_param, local_param in zip(
            self.net_.parameters(), self.net.parameters()
        ):
